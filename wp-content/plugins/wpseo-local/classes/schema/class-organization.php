...
class WPSEO_Local_Organization extends Abstract_Schema_Piece {
	 ...
	 *
	 * @return mixed|array Object containing location data.
	 */
	 ...
	 *
	 * @return bool|array Array with Organization schema data.
	 */
	...
	private function add_organization_attributes( $data, $location ) {
		...
		if ( ! empty( $location->business_phone ) ) {
			$data['telephone'][] = $location->business_phone;
		}

		if ( ! empty( $location->business_phone_2nd ) ) {
			$data['telephone'][] = $location->business_phone_2nd;
		}
	...
	private function fill_with_shared_business_info( $data ) {
		...
		// Add telephone numbers.
		if ( ! empty( $options['location_phone'] ) ) {
			$data['telephone'][] = $options['location_phone'];
		}

		if ( ! empty( $options['location_phone_2nd'] ) ) {
			$data['telephone'][] = $options['location_phone_2nd'];
		}
